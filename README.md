...
vim에서 `.(dot)` 을 활용하여 이전 명령을 바로 반복할 수 있지만, 이전 명령을 반복할 뿐 원하는 특정 작업에 대해서는 반복이 불가하기 때문에 매크로는 유용하게 사용될 수 있다.
...
앞서 설명했던 것처럼, 녹화를 시작하기 위해서는 **일반 모드**에서 `q` 와 매크로를 등록할 KEY를 입력해주면 된다

일반 모드에서 `q + KEY`를 누르면 vim의 상태바에 *Recording* 이라는 표시가 나오는데, 녹화가 시작되었다는 표시이다.
...
<img width="552" alt="image" src="https://user-images.githubusercontent.com/84072084/171694329-7d3213eb-eee6-4e5e-a1f5-1ff1e5c5bd70.png">

이 상태에서 원하는 매크로 작업을 해주면 된다

나는 예시를 들어, 각 줄의 첫번째 단어를 **test** 로 바꾸는 매크로를 작성해 보겠다.
...
<img width="547" alt="image" src="https://user-images.githubusercontent.com/84072084/171695121-67a0728c-3ce8-4a32-a7a5-4c0185412816.png">

첫 줄의 *hello*가 *test*로 변경되었고, 다음 줄의 첫번째 칸으로 이동하였다.
...
q
```
`q`를 입력하면 앞서 말했듯 지금까지 입력했던 매크로가 저장되고, 앞서 저장한 KEY를 이용하여 매크로를 실행시킬 수 있는 상태이다
...
5@a
```
바꾸어야 할 줄이 총 5줄이므로, 5번 반복하면 처음부터 끝까지 문장의 첫번째 단어는 **test**로 바뀔 것이다

<img width="542" alt="image" src="https://user-images.githubusercontent.com/84072084/171695595-75a09077-78b4-475c-982e-6df8a6f11231.png">

`[반복횟수]@[KEY]` 를 입력하면 횟수번 만큼 매크로를 반복할 수 있다.

<br>

## 알게된 사실

매크로를 만들고, 이를 파일로 저장해 두면 저장된 매크로를 불러올 수 있다
...
다음과 같이 만들어 두면, 입력 모드에서 `CTRL+r CTRL+r (매크로 KEY)` 를 동작하여 저장된 매크로를 불러올 수 있다.
...
사실, 블로그들이 워낙 발전해있고 편리하게 글을 쓸 수 있는 에디터들이 많은 시점에서 Markdown 문법으로 작성한다는 것은 쉽지많은 않은 일이지만 Markdown 또한 다른 블로그 에디터들에 비해서 장점들이 많다는 사실을 알게 되었던 것 같다.

많은 자료들을 조사하고, 정리하면서 이런것들이 귀찮기는 하지만 자료로 남는다는 것은 언제든 다시 볼수 있다는 것이기도 하기에 뿌듯하다는 느낌도 받을 수 있었고 배웠던 내용들을 다시금 상기시키는 계기도 된 것 같다.

앞으로도 깃허브는 많이 쓰이고, 없어서는 안될 필수적인 존재가 된 것 같은데 깃허브를 사용하면서 마크다운 문법은 뗄 수 없는 관계인 만큼 익숙해지고 편해진다면 잘 사용할 수 있을 것이라고 생각하는 계기가 되었다.
